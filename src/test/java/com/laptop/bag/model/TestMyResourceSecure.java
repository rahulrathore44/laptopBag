...
import com.laptop.laptopbag.MyResourceSecure;

public class TestMyResourceSecure extends JerseyTest {
	
	@Override
	protected ResourceConfig configure() {
		return new ResourceConfig(MyResourceSecure.class);
	}
	...
	public void testGetAllEndPoint(){
		Response response = target("/secure/all").request().get();
	...
	public void testGetEndPointWithId(){
		Response response = target("/secure/find/1").request().get();
		Assert.assertEquals(200, response.getStatus());
	...
	public void testGetEndPointWithIdNonExiting(){
		Response response = target("/secure/find/2").request().get();
		Assert.assertEquals(404, response.getStatus());
	}
	
	@Test
	public void testPingEndPoint(){
		Response response = target("/secure/ping/Hello").request().get();
		Assert.assertEquals(200, response.getStatus());
